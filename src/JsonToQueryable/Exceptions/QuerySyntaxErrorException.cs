namespace JsonToQueryable.Exceptions
{
    ...
    public class QuerySyntaxErrorException : Exception
    {
        public QuerySyntaxErrorException(string description, ISource source, int location)
        ...
        private static string ComposeMessage(string description, ISource source, int loc)
        {
            ...
            return $"Syntax Error Query ({location.Line}:{location.Column}) {description}" +
